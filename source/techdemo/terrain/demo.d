...
{
protected:
    AudioSampleSource mAmbienceSource;
    ...
    Entity createTerrain()
    {
        ...
        properties.size = Vector2f(256, 256);
        ...
        properties.blendMap = AssetManager.load!Texture2D("res://terraindemo/terrain/blendmap.png");
        properties.textureTiling = Vector2f(10);
    ...
    Entity createCamera(Vector3f position, quat rotation)
    {
...
public:
    this(GameStateApplication application)
    {
        ...
        createCamera(Vector3f(128f, 5f, 128f), quat.identity);
        ...
        for (uint y; y < entitymap.size.y; ++y)
            ...
            {
                immutable Color pixel = entitymap.getPixel(Vector2ui(x, y));
    ...
    override void onAttach(bool firstTime)
    {
        ...
        if (firstTime)
        {
            mAmbienceSource = new AudioSampleSource(null);
            mAmbienceSource.loop = true;
            mAmbienceSource.buffer = AssetManager.load!Sound("res://terraindemo/ambience.ogg");
        }
